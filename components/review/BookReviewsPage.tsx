...
// Added Pin for the highlighted UI
import { Star, Image as ImageIcon, Send, Loader2, Trash2, Paperclip, Pencil, ChevronDown, ChevronUp, Heart, Bookmark, X, BookMarked, Archive, Share2, Pin } from "lucide-react"; 
import { useAuthStore } from "@/store/useAuthStore";
import { getBookReviews, createReviewAction, deleteReviewAction, updateReviewAction, toggleReviewLike, toggleReviewFavorite, incrementReviewShare } from "@/services/review.service";
import { createClient } from "@/lib/supabase/client";
import { toast } from "sonner";
import { useSearchParams } from "next/navigation"; // Added for shared link detection

const BookReviewsPage = () => {
  const searchParams = useSearchParams();
  const highlightedId = searchParams.get("id"); // Get ID from URL
  ...
  // --- LOGIC: Sort highlighted review to the top ---
  const filteredReviews = useMemo(() => {
    let list = viewMode === "fav" ? reviews.filter(r => r.isFavorited) : reviews;

    if (highlightedId && list.length > 0) {
      const highlightedIdx = list.findIndex(r => r.id === highlightedId);
      if (highlightedIdx > -1) {
        const result = [...list];
        const [highlightedItem] = result.splice(highlightedIdx, 1);
        return [highlightedItem, ...result];
      }
    }
    return list;
  }, [reviews, viewMode, highlightedId]);
const handleShare = async (rev: any) => {
    const shareUrl = `${window.location.origin}/reviews?id=${rev.id}`;
    const shareData = {
      title: `Review: ${rev.bookTitle}`,
      text: `Check out this reflection on "${rev.bookTitle}" by ${rev.authorName} on BookPulse.`,
      url: shareUrl,
    };
    ...
    try {
      ...
      await incrementReviewShare(rev.id);
      fetchReviews();
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-6">
      {/* ... (Keep Header and Form exactly as is) ... */}
      ...
      <div className="space-y-12">
        {filteredReviews.length > 0 ? filteredReviews.map((rev) => {
          const isHighlighted = rev.id === highlightedId;
          
          return (
            <article 
              key={rev.id} 
              className={`relative bg-white dark:bg-[#252525] border-2 ${isHighlighted ? "border-[#d4a373] ring-4 ring-[#d4a373]/10" : "border-tertiary/5"} p-6 md:p-10 shadow-sm flex flex-col md:flex-row gap-8 group transition-all hover:shadow-md`}
            >
              {/* Highlight Badge */}
              {isHighlighted && (
                <div className="absolute -top-3 right-8 bg-[#d4a373] text-white px-3 py-1 text-[9px] font-mono font-black uppercase flex items-center gap-1 shadow-md animate-in slide-in-from-right-4 duration-500">
                  <Pin size={12} className="rotate-45" /> Linked Entry
                </div>
              )}

              <div className={`absolute top-0 left-0 w-1.5 h-full ${isHighlighted ? "bg-[#d4a373]" : "bg-[#d4a373] opacity-30"}`} />
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="relative">
                  <p className={`font-serif italic text-lg text-[#1a3f22]/80 dark:text-stone-200 leading-relaxed whitespace-pre-wrap ${(!expandedIds.has(rev.id) && !isHighlighted) ? "line-clamp-3" : ""}`}>
                      "{rev.content}"
                  </p>
                  {rev.content.length > 220 && !isHighlighted && (
                      <button onClick={() => toggleExpand(rev.id)} className="mt-2 text-tertiary dark:text-[#d4a373] font-mono text-[10px] font-black uppercase tracking-widest flex items-center gap-1 hover:underline">
                ...
                <div className="mt-auto pt-6 border-t border-dashed border-[#1a3f22]/10 flex justify-between items-center">
                  ...
                  <div className="flex gap-4">
                     ...
                     <button onClick={() => handleShare(rev)} className={`flex items-center gap-1 text-[10px] font-bold ${isHighlighted ? "text-[#d4a373]" : "text-stone-400 hover:text-tertiary"} transition-colors`}>
                        <Share2 size={16} /> {rev.shareCount || 0}
                     </button>
            ...
            <div className="py-32 text-center border-2 border-dashed border-[#d6c7a1]">
                ...
                <p className="font-serif italic text-[#8b5a2b]">No records found.</p>
            </div>
        )}
      </div>
    </div>
